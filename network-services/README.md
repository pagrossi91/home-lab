...
These services were set up with lots of google-foo. This README was generated by Claude and manually checked and manually revised for clarity and LLM-induced chaos.

## 🏗️ Architecture Overview

Pi-Hole and DNSCrypt-Proxy are running in parallel on two separate systems, the main homelab server and a Raspberry Pi 3B+. This is for redundancy. [Nebula-Sync](https://github.com/lovelaze/nebula-sync) is used to keep the Pi-Hole configurations in sync. A Home Assistant group is used to toggle both pi-holes on and off at the same time. Each DNS server configuration entry in the router direct to one of these Pi-Hole servers.
...
### Prerequisites
1. Docker and Docker Compose installed. Use Docker’s install script
    ```bash
      curl -fsSL https://get.docker.com -o get-docker.sh
      sudo sh get-docker.sh
    ```
    Then add manage Docker as a non-root user by following [Docker's installation guide](https://docs.docker.com/engine/install/linux-postinstall/). This eliminates the need for running `sudo docker ...` and can just use `docker ...`. 
    
    For more security, instead run Docker in rootless mode by following [Docker's Rootless Mode instructions](https://docs.docker.com/engine/security/rootless/). Note that in rootless mode, anywhere below where there is a `docker` command, you'll need to add the `sudo` preface.
...
# Pi-hole sync
PIHOLE_1_URL=http://192.168.50.XX:81
PIHOLE_1_PWD=${PIHOLE_WEBPASSWORD}
PIHOLE_2_URL=http://192.168.50.XX:81
...
**Function**: Keeps multiple pi-hole configurations synced. 

**Prerequisites**: Two or more pi-holes set up on a LAN.

#### Docker Compose Configuration
  ```yaml
  nebula-sync: # Sync pi-holes
    image: ghcr.io/lovelaze/nebula-sync:latest
    container_name: nebula-sync
    environment:
      TZ: ${TZ}
      PRIMARY: ${PIHOLE_1_URL}|${PIHOLE_1_PWD}
      REPLICAS: ${PIHOLE_2_URL}|${PIHOLE_2_PWD}
      FULL_SYNC: true
      CRON: 0 * * * *
      RUN_GRAVITY: true
  ```
  **Critical Environment Variables**:
  ```yaml
      PRIMARY: ${PIHOLE_1_URL}|${PIHOLE_1_PWD}
      REPLICAS: ${PIHOLE_2_URL}|${PIHOLE_2_PWD},${PIHOLE_3_URL}|${PIHOLE_3_PWD}
      CRON: 0 * * * *
      FULL_SYNC: true

  ```

  - **Primary**: Establishes the primary Pi-Hole from which to pull the configuration settings for the other Pi-Holes. When making configuration changes, use this one so the changes propagate effectively to the others. If you don't, changes made to other Pi-Holes will be undone during the next sync. Additionally, in this case, where the host webport is configured to be 81 (versus the default of 80), you must include the port in the environment variable (E.g., `http://192.168.50.XX:81`)
  - **REPLICAS**: The additional Pi-Holes that will receive the same settings as the Primary. For more than one replica Pi-Hole, separate them with commas.
  - **CRON**: Declare when or how often the sync occurs
  - **FULL_SYNC**: Sync all settings. This seems cause replica pi-holes to be unresponsive for a few moments during the sync. You can optionally set this to `false` and manually state which settings to sync in additional environment variables. See [nebula-sync](https://github.com/lovelaze/nebula-sync) documentation for details.
